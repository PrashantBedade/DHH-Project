...
def predict():
    try:
        ...
        user_input = data.get('symptoms', [])  # Fetch the symptoms list
        
        # Ensure user_input is a list
        if not isinstance(user_input, list):
            return jsonify({"error": "Invalid input, symptoms should be a list"}), 400

        # Process symptoms
        symptoms = [symptom.strip() for symptom in user_input if symptom.strip()]
        
        # Add prediction logic here
        prediction = "Some prediction based on symptoms"  # Placeholder
